...
export class ItemColorBox extends Component {
    ...
    startEvent(){
        this.nodeBox.on(Node.EventType.TOUCH_MOVE, (event: EventTouch) => {
            ...
            if(loc.x < (this.defaultWorldPos.y - GameGlobal.widthBox * 0.8) && this.bottomId == -1) this.setDefaultPos();
        })
        ...
        this.nodeBox.on(Node.EventType.TOUCH_CANCEL, (event: EventTouch) => {
            ...
            if(loc.x < (this.defaultWorldPos.x - GameGlobal.widthBox * 0.5) && loc.x > (this.defaultWorldPos.x - (GameGlobal.widthBox*1.5))) this.changePosLeft();
            if(loc.y > (this.defaultWorldPos.y + GameGlobal.widthBox * 0.5) && loc.y < (this.defaultWorldPos.y + (GameGlobal.widthBox*1.5))) this.changePosTop();
            if(loc.x > (this.defaultWorldPos.x - GameGlobal.widthBox * 0.5) && loc.x > (this.defaultWorldPos.y - (GameGlobal.widthBox*1.5))) this.changePosBottom();
        })
    }
    ...
    getSidePos(){
        ...
        // bottom
        if(this.idBox >= (GameGlobal.colomX**2)-1) this.topId = this.idBox + GameGlobal.colomX;
